...
int main(int argc, char* argv[])
{
	...
	buf  = malloc(BUFSIZ * nssid);
	size = malloc(sizeof *size * nssid);
	for (i = 0; i < nssid; ++i)
		size[i] = beacon(buf + i * BUFSIZ, vssid + i * len);
	...
	for (i = 0; (tsft(buf + i * BUFSIZ), pcap_inject(dev, buf + i * BUFSIZ, size[i])) >= 0; i = (i + 1) % nssid)
	{
		usleep(1000);
	}
